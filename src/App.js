...
function App() {
  ...
  const formSchema = yup.object().shape({
    ...
    name: yup.string().required("Nome obrigatório").matches(/^[A-Za-zÀ-ü ]{0,18}$/, "nome muito grande"),
    email: yup.string().required("E-mail obrigatório").email("email invalido"),
    confirmEmail: yup.string().required("Confirmação de e-mail obrigatório").email("email invalido"),
    ...
    <div className="App">
      <header className="App-header">
       <form  className ="form" onSubmit={handleSubmit(onSubmitFunction)}>
          <input className ="input" placeholder="nome de usuario" {...register("userName")}/>
          ...
          <input className ="input" placeholder="confirme o e-mail"{...register("confirmEmail")}/>
          <p>{errors.confirmEmail?.message}  </p> 
          <div className="password">
          <input className ="input-password" placeholder="senha" type="password" {...register("password")}/> 

          <input className ="input-password" placeholder="confirme a senha" type="password" {...register("confirmPassword")}/> 
